...
def main(message):
    ...
    if msg == '/start':
        ...
        send(id, 'ПРИМЕР РАССЧЁТА ЦЕНЫ: Допустим вам надо сделать фотосессию в парке, то: 1)Одна фотосессия - 20 фото бесплатно без обработки = 30 мин = 300 руб + с 31 мин цена = 10 руб/мин. 2)Для парка жанр фотографирования "уличная фотосъемка". За этот жанр цена 160 руб. В итоге: 450 руб до начала фотосессии + 10 руб/мин с 31 мин фотосессии, если не уложимся в полчаса и + цена за дополнительные фотки, если хотите больше 20 фоток. Если хотите с обработкой, то 50 руб/фото без предоплаты. 3)Если фотосессия нужна в платных парках (Дендропарки типо Ботанический сад) или внутри Москва Сити на 25 этаже, или на Эйфелевой башне и т.п., то за вход фотографа в эти места платит заказчик фотосессии.')
    elif msg == 'Обо Мне':
...
def reg_name(message):
    ...
    bot.send_message(chat_id = 1233877371, text = "У вас новая завка - " + " " + "Место: " + place + "; Время: " + time + "; Контакт: " + name + " " + contact)
